...
const Store = Object.assign({}, EventEmitter.prototype, {
  ...
  loadData(required) {
    required.forEach((params) => {
      Store.load(params.namespace, params.type, params.id, params.get);
    });
  },
  load(namespace, type, id, getter) {
    ...
    if (!(itemInStore && itemInStore.id === id)) {
      ...
      fetcher({
        ...
        url: type + (id ? '/'+id : ''),
        success: (data) => {
          Store.applyData(CamelCase(id ? Pluralize(type, 1) : type), getter ? getter(data) : data);
        }
      });
    } else {
