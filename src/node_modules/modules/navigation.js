...
export default class Navigation extends React.Component {
  ...
  render() {
    const navigationLinks = this.props.pages.map((link) => {
      return (
        <NavigationLink key={link.url} url={link.url} colour={link.colour} onClick={this.closeOverlay} selected={link.id === this.props.page}>
          {link.title}
        </NavigationLink>
      );
    });
    const navigationOverlayLinks = this.props.pages.map((link) => {
      return (
        <NavigationOverlayLink key={link.url} url={link.url} onClick={this.closeOverlay} selected={link.id === this.props.page}>
          {link.title}
    ...
    return (
      <header className={classnames('header', this.props.page, {
