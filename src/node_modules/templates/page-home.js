...
export default class PageHome extends React.Component {
  ...
  handleScroll = (e) => {
    const page = React.findDOMNode(this);
    const pageHeight = page.clientHeight;
    const parent = e.target;
    const scrollTop = parent.scrollTop;
    if(!this.scrollState.top && scrollTop >= parent.clientHeight) {
      this.scrollState.top = true;
      ga('send', 'event', 'hub_page', 'scroll_1', 'home');
    }
    if(!this.scrollState.middle && scrollTop >= ((page.clientHeight/2) - (parent.clientHeight/2))) {
      this.scrollState.middle = true;
      ga('send', 'event', 'hub_page', 'scroll_2', 'home');
    }
    if(!this.scrollState.bottom && scrollTop >= ((page.clientHeight) - (parent.clientHeight))) {
  ...
  scrollDownPage = (event) => {
    TweenMax.to(React.findDOMNode(this).parentElement, 0.8, {scrollTo:{y:this.state.blocks[0].blockReference().offsetHeight}, ease:Power2.easeOut});
  }
  setupScrollMagic = () => {
    ...
    let scrollSceneWelcome = new ScrollMagic.Scene({
      ...
      .on('progress', (e) => {
        ...
        if (progressRatio > 0) {
          this.refs.downChevron.goToProgressRatio(0.91 - progressRatio * 0.9);
        }
    });
  ...
  componentDidMount() {
    React.findDOMNode(this).parentElement.addEventListener('scroll', this.handleScroll);
  ...
  componentWillUnmount() {
    React.findDOMNode(this).parentElement.removeEventListener('scroll', this.handleScroll);
