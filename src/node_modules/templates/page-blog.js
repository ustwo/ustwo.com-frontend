...
const posts = [{
    ...
    "excerpt": {
        "rendered": "<p>Zombie ipsum reversus ab viral inferno, nam rick grimes malum cerebro. De carne lumbering animata corpora quaeritis. Summus brains sit​​, morbo vel maleficia? De apocalypsi gorger omero undead survivor dictum mauris.</p>"
    },
    "author": 4,
    "featured_image": 0,
    "comment_status": "open",
    ...
    "title": {
        "rendered": "Move Fast, Make Things: Lessons From Designing &#038; Building For Google Cardboard"
    },
    "content": {
        "rendered": ""
    },
    ...
    "type": "post",
    "category": "Apps, Design, innovation, Process, Product, UX",
    ...
    return (
      <article className="page-blog">
        <section className="hero">
          ...
          <DownChevron customClass="hero__down-chevron" ref="downChevron" />
        </section>
        <section className="blog-post-list">
          {/*get(pageData, 'page_builder.2.attr.posts.value', '').split(',').map((postName, index) => {
            const postData = find(get(pageData, '_embedded.ustwo:posts', []), 'slug', postName);
            const attachments = (pageData && pageData._embedded && pageData._embedded['http://v2.wp-api.org/attachment']) || [];
            return <BlogPostListItem key={postName} className="blog-post-list-item" featured={index === 0} data={postData} attachments={attachments} />;
          })*/}
          {posts.map((postData, index) => {
            const attachments = (pageData && pageData._embedded && pageData._embedded['http://v2.wp-api.org/attachment']) || [];
            return <BlogPostListItem key={postData.slug} className="blog-post-list-item" featured={index === 0} data={postData} attachments={attachments} />;
          })}
        </section>
      </article>
    );
